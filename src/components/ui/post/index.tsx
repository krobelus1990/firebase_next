...
export const Post: React.FC<PostProps> = ({ postList, userProfiles }) => {
  const [searchValue, setSerachValue] = useState('')
  const [inputedValue, setInputedValue] = useState('')
  const [postLists, setPostLists] = useState<any[]>(postList)
  const [postUser, setPostUser] = useState<any[]>(userProfiles)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (searchValue) {
        ...
        setPostUser(searchedPostList.searchedUsers);
      } else {
        setPostLists(postList);
        setPostUser(userProfiles);
      }
  ...
  return (
    <div className="px-5 pt-[30px] pb-0 bg-background shadow-md">
      ...
      <div className="flex flex-col bg-white gap-7">
        ...
        <div>
          <PostItem postList={postLists} userProfiles={postUser} />
        </div>
      </div>
    </div>
  );
